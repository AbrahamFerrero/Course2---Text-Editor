...
public class EfficientDocument extends Document {
	...
	private void processText()
	{
		...
		for (String a : tokens) {
			if(isWord(a)== true) {
				numWords++;
				numSyllables = numSyllables + countSyllables(a);
			}
			else {
				numSentences++;
			}
		}
		//If the sentence does not end with a sentence-ending punctuation:
		if(tokens.size()>0 && isWord(tokens.get(tokens.size()-1))) {numSentences++;}
