...
public abstract class Document {
	...
	protected List<String> getTokens(String pattern) {
		return getTextTokens(text, pattern);
	}

	private List<String> getTextTokens(String text, String pattern) {
	...
	protected int countSyllables(String word) {
		...
		int consecutiveVowels = getTextTokens(word, "[aeiouy]+").size();
		int nonConsecutiveVowels = getTextTokens(word, "[^aeiouy]*[aeiouy][^aeiouy]*e\\b").size();

		if (word.charAt(word.length() - 1) == 'e' && isAVowel(word.charAt(word.length() - 2))) {
			consecutiveVowels += 1;
		}

		return consecutiveVowels - nonConsecutiveVowels;
	}
